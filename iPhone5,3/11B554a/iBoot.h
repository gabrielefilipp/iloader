...
#define breakpoint1_ADDR        (0x1982C + 1) /* ResolvePathToCatalogEntry */

#define fuck1_ADDR              (0x1A68E + 1)
#define fuck2_ADDR              (0x1A6A4 + 1)
#define fuck3_ADDR              (0x1A7BA + 1)
...
#define readp_ADDR              (0x1A454 + 1)
#define get_mem_size_ADDR       (0x1FE84 + 1)
#define putchar_ADDR            (0x3457C + 1)
#define adjust_stack_ADDR       (0x1F660 + 1)
#define adjust_environ_ADDR     (0x1FB64 + 1)
#define disable_interrupts_ADDR (0x352C4 + 1)
#define cache_stuff_ADDR        (0x22E80 + 1)
...
#define panic_ADDR              (0x20D28 + 1)
#define system_init_ADDR        (0x20E14 + 1)
#define task_create_ADDR        (0x21444 + 1)
#define task_start_ADDR         (0x215A4 + 1)
#define task_exit_ADDR          (0x215C8 + 1)
#define printf_ADDR             (0x33DE4 + 1)
#define malloc_ADDR             (0x1A470 + 1)
#define free_ADDR               (0x1A524 + 1)
#define create_envvar_ADDR      (0x18D94 + 1)
#define bcopy_ADDR              (0x34868)
#define decompress_lzss_ADDR    (0x25E70 + 1)
...
if (1) {
    ...
    switch (seq) {
        ...
        case 2:
            ...
            PUT_DWORD_LE(buffer, 0x48B68 + 0x44 - 0x48B54, (uintptr_t)image + 0x48CC4); /* r11 -> lr */
#if 0
            ...
            PUT_DWORD_LE(buffer, 0x48BB0 +  42 - 0x48B54, make_bl(0, 0x48BB0 + 42, 0x21438));
            ...
            PUT_DWORD_LE(buffer, 0x48BB0 + 100 - 0x48B54, 0x42928 /* go command handler */);
            PUT_DWORD_LE(buffer, 0x48BB0 + 104 - 0x48B54, 0x1B0CC /* allow unsigned images */);
            PUT_DWORD_LE(buffer, 0x48BB0 + 108 - 0x48B54, INSN2_MOV_R0_0__STR_R0_R3 /* allow unsigned images */);
            PUT_DWORD_LE(buffer, 0x48BB0 + 112 - 0x48B54, (uintptr_t)image + 0x48000 /* nettoyeur uncompressed */);
...
{
    eprintf("irq_disable_count = 0x%x\n", *(unsigned int *)(image + 0x57400 + 0x18));
}
...
dispatch_SEGV(void *si_addr, _STRUCT_ARM_THREAD_STATE *tctx)
{
    ...
    if (si_addr == 0) {
        if (tctx->__pc == (uintptr_t)(image + 0x21256)) {
            /* idle task crap (read from *0) */
            tctx->__r[0] = *(uint32_t *)(image);
            tctx->__pc += 2;
            return 0;
        }
        if (tctx->__pc == (uintptr_t)(image + 0x214B6)) {
...
patch_image(unsigned char *image)
{
    ...
    *(uint32_t *)(image + 0x20E20) = INSN2_LDR_R1_PC__B_PLUS4;
    *(void **)(image + 0x20E24) = XLAT(IMAGE_END + IMAGE_HEAP_SIZE);
    /* clean data cache */
    *(uint32_t *)(image + 0x22A88) = INSNA_RETURN;
#if !USE_SIGNAL
    /* idle task crap (read from *0) */
    *(uint16_t *)(image + 0x21256) = INSNT_NOP;
    *(uint16_t *)(image + 0x214B6) = INSNT_MOV_R_R(1, 0);
#endif /* !USE_SIGNAL */
    /* timer 2 */
    *(uint32_t *)(image + 0x20684) = INSN2_RETURN_0;
    /* task switch FPU */
    *(uint32_t *)(image + 0x22EC4) = INSNA_MOV_R2_0;
    *(uint32_t *)(image + 0x22EF4) = INSNA_MOV_R2_0;
    /* bzero during mmu_init */
    *(uint32_t *)(image + 0x219F0) = INSN2_NOP__NOP;
    ...
    *(uint32_t *)(image + 0x1A08A) = INSN2_MOV_R0_1__MOV_R0_1;
    
    /* make main_task show SP */
    *(uint16_t *)(image + 0xDBC) = INSNT_MOV_R_R(1, 13);
    *(uint8_t *)(image + 0x356A4) = 'x';
    /* show task structure */
    *(void **)(image + 0xFCC) = image + 0x453A0;
    *(uint8_t *)(image + 0x35690) = 'x';
    ...
    *(uint16_t *)(image + 0x20806) = INSNT_NOP;
    *(uint16_t *)(image + 0x01960) = INSNT_NOP;
    *(uint32_t *)(image + 0x1FD94) = INSN2_RETURN_0;
    *(uint32_t *)(image + 0x1FEFA) = INSN2_RETURN_0;
    *(uint32_t *)(image + 0x1FF1A) = INSN2_RETURN_0;
    *(uint32_t *)(image + 0x1FF54) = INSN2_RETURN_0;
    *(uint32_t *)(image + 0x1FF6C) = INSN2_RETURN_21;
    *(uint32_t *)(image + 0x1FF84) = INSN2_RETURN_0;
    
    /* nocache */
    *(uint32_t *)(image + 0x191A4) = INSNT_RETURN;
    *(uint32_t *)(image + 0x02CC8) = INSNT_RETURN;
    *(uint32_t *)(image + 0x02D54)= INSNT_RETURN;
}
