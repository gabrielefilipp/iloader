...
#define IMAGE_END               0xbff5356c
#define IMAGE_SIZE              (IMAGE_END - IMAGE_START)
#define IMAGE_HEAP_SIZE         0xA3A94
...
#define breakpoint1_ADDR        (0x19480 + 1) /* ResolvePathToCatalogEntry */

#define fuck1_ADDR              (0x1a2e2 + 1)
#define fuck2_ADDR              (0x1a2f8 + 1)
#define fuck3_ADDR              (0x1a40e + 1)
...
#define readp_ADDR              (0x1a0a8 + 1)
#define get_mem_size_ADDR       (0x1fac4 + 1)
#define putchar_ADDR            (0x34160 + 1)
#define adjust_stack_ADDR       (0x1f2a4 + 1)
#define adjust_environ_ADDR     (0x1f7a4 + 1)
#define disable_interrupts_ADDR (0x34ea0 + 1)
#define cache_stuff_ADDR        (0x22800 + 1)
...
#define panic_ADDR              (0x20968 + 1)
#define system_init_ADDR        (0x20a54 + 1)
#define task_create_ADDR        (0x21084 + 1)
#define task_start_ADDR         (0x211e4 + 1)
#define task_exit_ADDR          (0x21208 + 1)
#define printf_ADDR             (0x339c8 + 1)
#define malloc_ADDR             (0x1a0c4 + 1)
#define free_ADDR               (0x1a178 + 1)
#define create_envvar_ADDR      (0x189e8 + 1)
#define bcopy_ADDR              (0x3444c)
#define decompress_lzss_ADDR    (0x257f0 + 1)
...
#define NODE_SIZE 0x400 /* XXX a size this large will use cache for catalog blocks */
...
#define ALIGNED_POINTER_OFFSET 0x47B68
            ...
            PUT_DWORD_LE(buffer, 0x47BB0 +  42 - 0x47B54, make_bl(0, 0x47BB0 + 42, 0x21078));
            ...
            PUT_DWORD_LE(buffer, 0x47BB0 + 100 - 0x47B54, 0x42138 /* go command handler */);
            PUT_DWORD_LE(buffer, 0x47BB0 + 104 - 0x47B54, 0x1ad20 /* allow unsigned images */);
...
{
    eprintf("irq_disable_count = 0x%x\n", *(unsigned int *)(image + 0x56480 + 0x18));
}
...
dispatch_SEGV(void *si_addr, _STRUCT_ARM_THREAD_STATE *tctx)
{
    ...
    if (si_addr == 0) {
        if (tctx->__pc == (uintptr_t)(image + 0x20e96)) {
            /* idle task crap (read from *0) */
            tctx->__r[0] = *(uint32_t *)(image);
            tctx->__pc += 2;
            return 0;
        }
        if (tctx->__pc == (uintptr_t)(image + 0x210f6)) {
...
patch_image(unsigned char *image)
{
    ...
    *(uint32_t *)(image + 0x20a60) = INSN2_LDR_R1_PC__B_PLUS4;
    *(void **)(image + 0x20a64) = XLAT(IMAGE_END + IMAGE_HEAP_SIZE);
    /* clean data cache */
    *(uint32_t *)(image + 0x22408) = INSNA_RETURN;
#if !USE_SIGNAL
    /* idle task crap (read from *0) */
    *(uint16_t *)(image + 0x20e96) = INSNT_NOP;
    *(uint16_t *)(image + 0x210f6) = INSNT_MOV_R_R(1, 0);
#endif /* !USE_SIGNAL */
    /* timer 2 */
    *(uint32_t *)(image + 0x202c4) = INSN2_RETURN_0;
    /* task switch FPU */
    *(uint32_t *)(image + 0x22844) = INSNA_MOV_R2_0;
    *(uint32_t *)(image + 0x22874) = INSNA_MOV_R2_0;
    /* bzero during mmu_init */
    *(uint32_t *)(image + 0x21630) = INSN2_NOP__NOP;
    ...
    *(uint32_t *)(image + 0x19cde) = INSN2_MOV_R0_1__MOV_R0_1;
    ...
    *(uint16_t *)(image + 0x20446) = INSNT_NOP;
    *(uint16_t *)(image + 0x01968) = INSNT_NOP;
    *(uint32_t *)(image + 0x1f9d4) = INSN2_RETURN_0;
    *(uint32_t *)(image + 0x1fb3a) = INSN2_RETURN_0;
    *(uint32_t *)(image + 0x1fb5a) = INSN2_RETURN_0;
    *(uint32_t *)(image + 0x1fb94) = INSN2_RETURN_0;
    *(uint32_t *)(image + 0x1fbac) = INSN2_RETURN_21;
    *(uint32_t *)(image + 0x1fbc4) = INSN2_RETURN_0;
    
    /* nocache */
    *(uint32_t *)(image + 0x18df8) = INSNT_RETURN;
}
